#include "CEntity.h"

std::vector<CEntity*> CEntity::EntityList;

CEntity::CEntity() {
    Tex_Entity = NULL;
    
    X = 0;
    Y = 0;
    
    Width = Height = 0;
    
    AnimState = 0;
    
    MoveLeft = false;
    MoveRight = false;
    MoveUp = false;
    MoveDown = false;
    SpaceDown = false;
    
    string = NULL;
    print = false;
    
    Type = ENTITY_TYPE_GENERIC;
    
    Dead = false;
    Flags = ENTITY_FLAG_NONE;
    
    SpeedX = 0;
    SpeedY = 0;
    
    CurrentFrameCol = 0;
    CurrentFrameRow = 0;
    
    Col_X = 0;
    Col_Y = 0;
    
    Col_Width = 0;
    Col_Height = 0;
}

CEntity::~CEntity() {
}

bool CEntity::OnLoad(SDL_Renderer* Rend, char* File, int Width, int Height, int MaxFrames) {
    if ((Surf_Entity = CTexture::OnLoad(File)) == NULL)
    {
        return false;
    }
    
    CTexture::Transparent(Surf_Entity, 255, 0, 255);
    Tex_Entity = CTexture::S2T(Rend, Surf_Entity);
    
    this->Width = Width;
    this->Height = Height;
    
    Anim_Control.MaxFrames = MaxFrames;
    
    return true;
}

void CEntity::OnLoop() {
    if (MoveLeft == false && MoveRight == false && MoveUp == false && MoveDown == false)
    {
        StopMove();
    }
    
    if (MoveDown && MoveRight)
    {
        SpeedX = 5;
        SpeedY = 5;
    }
    else if (MoveDown && MoveLeft)
    {
        SpeedX = -5;
        SpeedY = 5;
    }
    else if (MoveUp && MoveRight)
    {
        SpeedX = 5;
        SpeedY = -5;
    }
    else if (MoveUp && MoveLeft)
    {
        SpeedX = -5;
        SpeedY = -5;
    }
    else if (MoveLeft)
    {
        SpeedX = -5;
        SpeedY = 0;
    }
    else if (MoveRight)
    {
        SpeedX = 5;
        SpeedY = 0;
    }
    
    else if (MoveUp)
    {
        SpeedY = -5;
        SpeedX = 0;
    }
    else if (MoveDown)
    {
        SpeedY = 5;
        SpeedX = 0;
    }
    
    OnAnimate();
    OnMove(SpeedX, SpeedY);
}

void CEntity::OnRender(SDL_Renderer* Rend) {
    if (Rend == NULL || Tex_Entity == NULL)
    {
        return;
    }
    
    CTexture::OnDraw(Rend, Tex_Entity, X - CCamera::CameraControl.GetX(), Y - CCamera::CameraControl.GetY(), CurrentFrameCol * Width, (CurrentFrameRow + Anim_Control.GetCurrentFrame()) * Height, Width, Height);
}

void CEntity::OnCleanup() {
    if (Tex_Entity)
    {
        SDL_DestroyTexture(Tex_Entity);
    }
    Tex_Entity = NULL;
}

void CEntity::OnAnimate() {
    if (MoveLeft)
    {
        CurrentFrameCol = 0;
    }
    else if (MoveRight)
    {
        CurrentFrameCol = 1;
    }
    else if (MoveUp)
    {
        CurrentFrameCol = 2;
    }
    else if (MoveDown)
    {
        CurrentFrameCol = 3;
    }
    
    Anim_Control.OnAnimate();
}

bool CEntity::OnCollision(CEntity* Entity) {
}

void CEntity::OnMove(float MoveX, float MoveY) {
    if (MoveX == 0 && MoveY == 0) return;
    
    double NewX = 0;
    double NewY = 0;
    
    MoveX *= CFPS::FPSControl.GetSpeedFactor();
    MoveY *= CFPS::FPSControl.GetSpeedFactor();
    
    if (MoveX != 0)
    {
        if (MoveX >= 0)
        {
            NewX = CFPS::FPSControl.GetSpeedFactor();
        }
        else
        {
            NewX = -CFPS::FPSControl.GetSpeedFactor();
        }
    }
    if (MoveY != 0)
    {
        if (MoveY >= 0)
        {
            NewY = CFPS::FPSControl.GetSpeedFactor();
        }
        else
        {
            NewY = -CFPS::FPSControl.GetSpeedFactor();
        }
    }
    
    while(true)
    { 
        if (PosValid((int)(X + NewX), (int)(Y)))
        {
            X += NewX;
        }
        else
        {
            SpeedX = 0;
        }
        
        if (PosValid((int)(X), (int)(Y + NewY)))
        {
            Y += NewY;
        }
        else
        {
            SpeedY = 0;
        }
        MoveX += -NewX;
        MoveY += -NewY;
        
        if(NewX > 0 && MoveX <= 0) NewX = 0;
        if(NewX < 0 && MoveX >= 0) NewX = 0;

        if(NewY > 0 && MoveY <= 0) NewY = 0;
        if(NewY < 0 && MoveY >= 0) NewY = 0;

        if(MoveX == 0) NewX = 0;
        if(MoveY == 0) NewY = 0;

        if(MoveX == 0 && MoveY == 0)     break;
        if(NewX  == 0 && NewY == 0)     break;
    }
}

void CEntity::StopMove() {
    SpeedX = 0;
    SpeedY = 0;
}

bool CEntity::Collides(int oX, int oY, int oW, int oH) {
    int left1, left2;
    int right1, right2;
    int top1, top2;
    int bottom1, bottom2;
    
    int tX = (int)X + Col_X;
    int tY = (int)Y + Col_Y;
    
    left1 = tX;
    left2 = oX;
    
    right1 = left1 + Width - 1 - Col_Width;
    right2 = oX + oW - 1;
    
    top1 = tY;
    top2 = oY;
    
    bottom1 = top1 + Height - 1 - Col_Height - 35;
    bottom2 = oY + oH - 1;
    
    if (bottom1 < top2) return false;
    if (top1 > bottom2) return false;
    if (right1 < left2) return false;
    if (left1 > right2) return false;
    
    return true;
}

bool CEntity::PosValid(int NewX, int NewY) {
    bool Return = true;
    
    int StartX = (NewX + Col_X) / TILE_SIZE;
    int StartY = (NewY + Col_Y) / TILE_SIZE;
    
    int EndX = ((NewX + Col_X) + Width - Col_Width - 1) / TILE_SIZE;
    int EndY = ((NewY + Col_Y) + Height - Col_Height - 1) / TILE_SIZE;
    
    for (int iY = StartY; iY <= EndY; iY++)
    {
        for (int iX = StartX; iX <= EndX; iX++)
        {
            CTile* Tile = CMap::MapControl.GetTile(iX* TILE_SIZE, iY * TILE_SIZE);
            
            if (PosValidTile(Tile) == false)
            {
                Return = false;
            }
        }
    }
    
    if (Flags & ENTITY_FLAG_MAPONLY)
    {
    }
    else
    {
        for (int i = 0; i < EntityList.size(); i++)
        {
            if (PosValidEntity(EntityList[i], NewX, NewY) == false)
            {
                Return = false;
            }
        }
    }
    return Return;
}

bool CEntity::PosValidTile(CTile* Tile)
{
    if (Tile == NULL)
    {
        return true; // you can walk on this tile
    }
    
    if (Tile->TypeID == TILE_TYPE_BLOCK)
    {
        return false; // you can not walk on this tile
    }
    return true;
}

bool CEntity::PosValidEntity(CEntity* Entity, int NewX, int NewY) {
    if (this != Entity && Entity != NULL && Entity-> Dead == false &&
        Entity->Flags ^ ENTITY_FLAG_MAPONLY && 
        Entity->Collides(NewX + Col_X, NewY + Col_Y, Width - Col_Width - 1, Height - Col_Height - 1) == true)
    {
        CEntityCol EntityCol;
        
        EntityCol.EntityA = this;
        EntityCol.EntityB = Entity;
        
        CEntityCol::EntityColList.push_back(EntityCol);
        
        return false;
    }
    
    return true;
}

bool CEntity::Print(CEntity* Entity) {
    if (Entity->string == NULL) return false;
    
    print = true;
    //printf("Text: %s\n", Entity->string);
    if (SpaceDown)
    {
        (SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Text", Entity->string, NULL) == true);
        SpaceDown = false;
        MoveLeft = false;
        MoveRight = false;
        MoveUp = false;
        MoveDown = false;
    }
    
    return true;
}

bool CEntity::Text(SDL_Renderer* Rend, CEntity* Entity) {
    if (print == false) return false;
    else if (print == true){
    SDL_Log("Hey");
    SDL_Texture* Texture = NULL;
    Texture = CText::Load(Rend, Entity->string);
    CTexture::OnDraw(Rend, Texture, 320, 400);
    }
    return true;   
}

